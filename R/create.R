...
suspect.by.qc <- function(DIRECTORY, TABLE = FALSE) {
  ...
  thour <-
    ...
    dplyr::mutate(
      data_type = "horário",
      tot_obs = paste0(tot_obs),
      suspect = paste0(suspect)) %>% 
    dplyr::select(qc, data_type, everything())
  ...
  names(p3) [1] <- "QC"
  names(p3) [2] <- "Tipo de dado"
  names(p3) [3] <- "Total de dados"
  names(p3) [4] <- "Suspeitos"
  names(p3) [5] <- "Suspeitos (em %)"
#  sbq <- knitr::kable(p3)
  sbq <- DT::datatable(p3)
...
#      perc_day = sum(perc_day, na.rm = TRUE)
  ...
  names(p5) [2] <- "Estado"
  names(p5) [3] <- "Nome"
  names(p5) [4] <- "Suspeitos* (em horas)"
  names(p5) [5] <- "Suspeitos** (em dias)"
#  sbs <- knitr::kable(p5)
  sbs <- DT::datatable(p5)
